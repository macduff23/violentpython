...
import socket
from socket import *
#We also need to support screenlocking through the threading modules
...
#Here we lock the screen when we get results
		screenLock.acquire()
...
#Also acquire screenlock during exception
		screenLock.acquire()
		print '[-]%d/tcp closed'% tgtPort
#And here we release the lock if it doesn't work so the other threads can proceed. 
	finally:
		screenLock.release()
		connSkt.close()
...
#Instead of the old bit, we're goping to thread it
...
#This is different now too apparently.
